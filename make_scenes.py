...
space = Space([
    ...
    (0.01, depth - air_space),  # cylinder depth
    (0, width),  # cylinder x position
...
for i, (radius, cylinder_depth, cylinder_x, rx_tx_height, surface_roughness) in enumerate(labels):
    ...
    cylinder_z = depth - air_space - cylinder_depth
    ...
    geo = Geometry(
        ...
        [cylinder],
        RickerWaveform(1, 1.5e9, 'ricker'),
        step_size=0.02,
        receiver_location=Point(spatial_resolution * 10, rx_tx_height, thickness / 2),
