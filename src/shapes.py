...
from materials import Material, MixingModel
...
class Shape:
    def __init__(self, material: Material, smooth: Boolean = False):
...
class Box(Shape):
    ...
    def __init__(self, lower_left: Point, upper_right: Point, material: Material, smooth: Boolean = False):
        super().__init__(material, smooth)
...
class Sphere(Shape):
    ...
    def __init__(self, center: Point, radius: Number, material: Material, smooth: Boolean = False):
        super().__init__(material, smooth)
        self.center = center
        self.radius = radius
...
class FractalBox(Box):
    def __init__(self, lower_left: Point, upper_right: Point, fractal_dimension: Number, weight_x: Number,
                 weight_y: Number, weight_z: Number, material: Material, identifier: str):
        """
    ...
    def __str__(self):
        ...
        return f"#fractal_box: {self.lower_left} {self.upper_right} {self.fractal_dimension} {self.weight_x} " \
...
The water is modelled using a single-pole Debye formulation with properties ϵrs=80.1, ϵ∞=4.9, and a relaxation time of τ=9.231×10−12 seconds (http://dx.doi.org/10.1109/TGRS.2006.873208). If you prefer, gprMax will use your own definition for water as long as it is named water.
#add_grass:
