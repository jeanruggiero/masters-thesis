...
def arraylist(header, data):
    ...
    img_arr = data[:header['rh_nchan'] * header[
...
def readdzt(infile, gps=DataFrame(), spm=None, start_scan=0, num_scans=-1,
    ...
    header['rh_zero'] = struct.unpack('<h', infile.read(2))[
    ...
    header['rhf_top'] = struct.unpack('<f', infile.read(4))[
    ...
    header['rhf_depth'] = header['dzt_depth'] * (math.sqrt(header['dzt_epsr']) / math.sqrt(
        header['rhf_epsr']))  # range based on user epsr, before subtracting rhf_top
    ...
    for chan in list(range(header['rh_nchan'])):
        ...
        try:
            ...
            try:
                header['antfreq'][chan] = int("".join(takewhile(str.isdigit,
                                                                header['rh_ant'][chan].replace('D5', '').replace('D6',
                                                                                                                 ''))))  # hoping this works
            except ValueError:
                header['antfreq'] = freq
    ...
    header['rh_version'] = int('{0:08b}'.format(ord(header['vsbyte']))[5:],
                               2)  # ord(vsbyte) >> 5 # whether or not the system is GPS-capable, 1=no 2=yes (does not mean GPS is in file)
    header['rh_system'] = int('{0:08b}'.format(ord(header['vsbyte']))[:5],
                              2)  # ord(vsbyte) >> 3 ## the system type (values in UNIT={...} dictionary in constants.py)
    ...
    for i in range(header['rh_nchan']):
        try:
            ...
            fx.printmsg('WARNING: no time zero specified for channel %s, defaulting to rh_zero value (%s)' % (
            i, header['rh_zero']))
    ...
    if os.path.isfile(infile_dzx):
        ...
        usr_marks = np.ndarray.tolist(data[
                                          1])  # when the system type is SIR3000, the second row should be user marks (otherwise these are in the DZX, see note below)
        i = 0
        for m in usr_marks:
            if m > 0:
                # print(m)
...
def header_info(header, data):
    ...
    if header['dzt_spm'] != header['rhf_spm']:
        fx.printmsg('traces per meter:   %.2f (manually set - value from DZT: %.2f)' % (
        float(header['rhf_spm']), float(header['dzt_spm'])))
    else:
        fx.printmsg('traces per meter:   %.1f' % (float(header['rhf_spm'])))
    if header['dzt_epsr'] != header['rhf_epsr']:
        fx.printmsg(
            'user epsr:          %.1f (manually set - value from DZT: %.1f)' % (header['rhf_epsr'], header['dzt_epsr']))
    else:
    ...
    if header['dzt_depth'] != header['rhf_depth']:
        fx.printmsg('sampling depth:     %.1f m (manually set - value from DZT: %.1f)' % (
